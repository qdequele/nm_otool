...
/*   Updated: 2018/02/06 11:59:04 by quentindequ      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_architecture(t_list *node)
{
	...
	if (magic_number == MH_MAGIC_64 || magic_number == MH_CIGAM_64)
		match_header((archi = create_architecture("64Bits", file->ptr)));
	else if (magic_number == MH_MAGIC || magic_number == MH_CIGAM)
		match_header((archi = create_architecture("32Bits", file->ptr)));
	else if (magic_number == FAT_MAGIC || magic_number == FAT_CIGAM)
		archi = create_architecture("FAT32", file->ptr);
	else if (magic_number == FAT_MAGIC_64 || magic_number == FAT_CIGAM_64)
		archi = create_architecture("FAT64", file->ptr);
	else if (ft_strncmp((char *)file->ptr, ARMAG, SARMAG) == 0)
	{
		...
		archi = create_architecture("UNDEFINED", file->ptr);
